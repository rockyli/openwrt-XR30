...
/* The parser hashes the tuple it matched and says which tuple that was. It
 * reuses the same field to hand back the index of an offloaded flow, which is
 * not a hash, so only the two tuple verdicts are taken.
 *
 ...
 * table on the other side. The misc status is left out because reading it is
 * what acknowledges an error the handler would otherwise report.
 */
...
/* The engine reads a frame from a bounded number of buffers and stops short on
 * one that is too small for it, so a frame the rings cannot describe is made
 * linear instead. The last buffer has no length floor, and the head grows by
 * the preheader before it is mapped, so neither is counted here.
 */
...
static int edma_probe(struct platform_device *pdev)
{
	...
	netif_set_tso_max_size(netdev, EDMA_TX_MAX_FRAME);
