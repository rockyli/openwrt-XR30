...
/* Replace a VID's port masks and select its L2 learning mode. */
static int rtl960x_vlan_set_masks(struct rtl960x_dsa *priv, u16 vid,
				  u32 member, u32 untag, bool ivl)
{
...
static void rtl960x_vlan_setup(struct dsa_switch *ds)
{
	...
	for (v = 0; v < RTL960X_NUM_VLANS; v++)
		rtl960x_vlan_set_masks(priv, v, 0, 0, false);

	rtl960x_vlan_set_masks(priv, RTL960X_DEFAULT_VID, all, all, false);
	...
	for (p = 0; p < ds->num_ports; p++) {
		if (!dsa_is_user_port(ds, p) && !dsa_is_cpu_port(ds, p))
			continue;
		regmap_write(priv->map, SWCORE_VLAN_EGR_TAG(p),
			     dsa_is_cpu_port(ds, p) ?
			     SWCORE_VLAN_EGR_MODE_KEEP : SWCORE_VLAN_EGR_MODE_ORI);
...
static void rtl960x_port_setup_standalone(struct dsa_switch *ds, int port)
{
	...
	rtl960x_vlan_set_masks(priv, vid, mask, mask, false);
				     ...
				     struct netlink_ext_ack *extack)
{
	 ...
	 */
	untag &= ~BIT(RTL960X_CPU_PORT);

	/* Match hardware learning to DSA's VID-keyed FDB add/delete operations. */
	ret = rtl960x_vlan_set_masks(priv, vlan->vid, member, untag, true);
	if (ret)
		return ret;
				     ...
				     const struct switchdev_obj_port_vlan *vlan)
{
	...
	ret = rtl960x_vlan_set_masks(priv, vlan->vid, member, untag, member != 0);
	if (ret)
		return ret;
...
struct rtl960x_l2_entry {
	u8 mac[ETH_ALEN];
	u16 vid;		/* CVID for IVL; 0 selects/reports SVL with FID 0 */
...
static void rtl960x_l2_unpack(struct rtl960x_l2_entry *e, const u32 *w)
{
	...
	/* CVID is not part of an SVL entry's key; DSA represents FID 0 as VID 0. */
